...
impl VerbosityAnnotations {
    ...
    /// Returns the verbosity for the given stream
    pub fn verbosity(&self, sr: StreamReference) -> StreamVerbosity {
        *self.stream_verbosity.get(&sr).unwrap()
    }

    /// Returns whether the given stream is marked as debug
    pub fn debug(&self, sr: StreamReference) -> bool {
