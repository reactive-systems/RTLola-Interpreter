...
impl TotalOp for Variance {
    fn instances<'s>(&'s mut self, instances: impl IntoIterator<Item = &'s InstanceStore>) {
        (self.sum, self.m_2, self.count) = instances
            .into_iter()
...
impl TotalOp for CoVar {
    fn instances<'s>(&'s mut self, instances: impl IntoIterator<Item = &'s InstanceStore>) {
        (self.sum_x, self.sum_y, self.co_moment, self.count) = instances
            .into_iter()
...
impl TotalOp for Percentile {
    fn instances<'s>(&'s mut self, instances: impl IntoIterator<Item = &'s InstanceStore>) {
        ...
        if count % 100 == 0 {
            ...
            self.res = values[idx].clone() + diff;
        }
    }
}
...
mod tests {
    ...
    fn test_count() {
        ...
        let mut fresh = FreshAggregation(Count::neutral(&FLOAT_TY));
        let mut all = Incremental(AllAggregation(Count::neutral(&FLOAT_TY)));
        apply_incremental(values, &mut all);
        fresh.all_instances(&store);

        assert_eq!(fresh.get_value(), Value::Unsigned(4));
        assert_eq!(all.get_value(), Value::Unsigned(4));
    }
    ...
    fn test_min() {
        ...
        let mut fresh = FreshAggregation(Min::neutral(&FLOAT_TY));
        let mut all = Total(AllAggregation(Min::neutral(&FLOAT_TY)));
        all.all_instances(&store);
        fresh.all_instances(&store);

        assert_eq!(fresh.get_value(), float(-13.0));
        assert_eq!(all.get_value(), float(-13.0));
    }
    ...
    fn test_max() {
        ...
        let mut fresh = FreshAggregation(Max::neutral(&FLOAT_TY));
        let mut all = Total(AllAggregation(Max::neutral(&FLOAT_TY)));
        all.all_instances(&store);
        fresh.all_instances(&store);

        assert_eq!(fresh.get_value(), float(42.0));
        assert_eq!(all.get_value(), float(42.0));
    }
    ...
    fn test_sum() {
        ...
        let mut fresh = FreshAggregation(Sum::neutral(&FLOAT_TY));
        let mut all = Incremental(AllAggregation(Sum::neutral(&FLOAT_TY)));
        apply_incremental(values, &mut all);
        fresh.all_instances(&store);

        assert_eq!(fresh.get_value(), float(41.0));
        assert_eq!(all.get_value(), float(41.0));
    }
    ...
    fn test_avg() {
        ...
        let mut fresh = FreshAggregation(Avg::neutral(&FLOAT_TY));
        let mut all = Incremental(AllAggregation(Avg::neutral(&FLOAT_TY)));
        apply_incremental(values, &mut all);
        fresh.all_instances(&store);

        assert_eq!(fresh.get_value(), float(10.25));
        assert_eq!(all.get_value(), float(10.25));
    }
    ...
    fn test_product() {
        ...
        let mut fresh = FreshAggregation(Product::neutral(&FLOAT_TY));
        let mut all = Incremental(AllAggregation(Product::neutral(&FLOAT_TY)));
        apply_incremental(values, &mut all);
        fresh.all_instances(&store);

        assert_eq!(fresh.get_value(), float(-19110.0));
        assert_eq!(all.get_value(), float(-19110.0));
    }
    ...
    fn test_conjunction() {
        ...
        let mut fresh = FreshAggregation(All::neutral(&BOOL_TY));
        let mut all = Incremental(AllAggregation(All::neutral(&BOOL_TY)));
        apply_incremental_cb(values, &mut all, |idx| Value::Bool(idx % 2 == 0));
        fresh.all_instances(&store);

        assert_eq!(fresh.get_value(), Value::Bool(false));
        assert_eq!(all.get_value(), Value::Bool(false));
    }
    ...
    fn test_disjunction() {
        ...
        let mut fresh = FreshAggregation(Any::neutral(&BOOL_TY));
        let mut all = Incremental(AllAggregation(Any::neutral(&BOOL_TY)));
        apply_incremental_cb(values, &mut all, |idx| Value::Bool(idx % 2 == 0));
        fresh.all_instances(&store);

        assert_eq!(fresh.get_value(), Value::Bool(true));
        assert_eq!(all.get_value(), Value::Bool(true));
    }
    ...
    fn test_variance() {
        ...
        let mut fresh = FreshAggregation(Variance::neutral(&FLOAT_TY));
        let mut all = Incremental(AllAggregation(Variance::neutral(&FLOAT_TY)));
        apply_incremental(values, &mut all);
        fresh.all_instances(&store);

        assert_eq!(fresh.get_value(), float(396.6875));
        assert_eq!(all.get_value(), float(396.6875));
    }
    ...
    fn test_stddev() {
        ...
        let mut fresh = FreshAggregation(StdDev::neutral(&FLOAT_TY));
        let mut all = Incremental(AllAggregation(StdDev::neutral(&FLOAT_TY)));
        apply_incremental(values, &mut all);
        fresh.all_instances(&store);

        assert_eq!(fresh.get_value(), float(19.917015338649513));
        assert_eq!(all.get_value(), float(19.917015338649513));
    }
    ...
    fn test_covariance() {
        ...
        let mut fresh = FreshAggregation(CoVar::neutral(&FLOAT_TY));
        ...
        assert_eq!(fresh.get_value(), float(153.8125));
        assert_eq!(all.get_value(), float(153.8125));
    }
    ...
    fn test_percentile() {
        ...
        assert_eq!(fresh.get_value(), float(6.0));
        assert_eq!(all.get_value(), float(6.0));
        ...
        assert_eq!(fresh.get_value(), float(0.5));
        assert_eq!(all.get_value(), float(0.5));
        ...
        assert_eq!(fresh.get_value(), float(15.75));
        assert_eq!(all.get_value(), float(15.75));
        ...
        assert_eq!(fresh.get_value(), float(5.52));
        assert_eq!(all.get_value(), float(5.52));
    }
}
